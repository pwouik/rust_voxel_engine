...
impl ChunkLoader {
    pub fn new() -> Self {
        ...
        let (load_sender, load_receiver) = crossbeam_channel::bounded(60)
            as (
                crossbeam_channel::Sender<Point3<i32>>,
                crossbeam_channel::Receiver<Point3<i32>>,
            );
        let (save_sender, save_receiver) = mpsc::channel()
            as (
                mpsc::Sender<(Point3<i32>, Box<Chunk>)>,
                mpsc::Receiver<(Point3<i32>, Box<Chunk>)>,
            );
        ...
        let storage_thread_handle = Some(thread::spawn(move || {
            loop {
                match save_receiver.try_recv() {
                    Ok(chunk) => {
                        let region_pos: Point3<i32> =
                            point3(chunk.0.x >> 4, chunk.0.y >> 2, chunk.0.z >> 4);
                    ...
                    _ => {
                        ...
                        loop {
                            match load_receiver.try_recv() {
                                Ok(pos) => {
                                    let region_pos: Point3<i32> =
                                        point3(pos.x >> 4, pos.y >> 2, pos.z >> 4);
