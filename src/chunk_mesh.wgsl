...
fn main([[builtin(global_invocation_id)]] global_id: vec3<u32>) {
    let block1_pos:u32 = global_id.x + 34u * global_id.z + 34u * 34u * global_id.y + 1u + 34u + 34u * 34u;
    let block1:u32 = blocks.data[block1_pos];
    let block2:u32 = blocks.data[block1_pos + 1u];
    ...
    let block2 = blocks.data[block1_pos + 34u * 34u];
    if(block1>0u && block2==0u){
        let index=atomicAdd(&counter.c,1u);
        mesh.data[index].pos_dir=global_id.x + ( global_id.y << 8u ) + ( global_id.z << 16u ) + ( 5u << 24u );
        mesh.data[index].texture=TEXTURE_INDEX[block1 * 6u - 1u];
        mesh.data[index].light=0xFFFFFFFFu;
    }
    else{
        if(block1==0u && block2>0u){
            ...
            mesh.data[index].texture=TEXTURE_INDEX[block2 * 6u - 2u];
            mesh.data[index].light=0xFFFFFFFFu;
        }
    }
}